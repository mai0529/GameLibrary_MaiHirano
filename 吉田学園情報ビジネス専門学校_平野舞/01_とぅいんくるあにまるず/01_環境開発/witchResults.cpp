...
{
	// オブジェクト2Dを生成
	m_paObject2D[0] = CObject2D::Create(D3DXVECTOR3(pos.x, pos.y, 0.0f), D3DXVECTOR3(RESULT_WIDTH, RESULT_HEIGHT, 0.0f));
	m_paObject2D[1] = CObject2D::Create(D3DXVECTOR3(pos.x + 700.0f, pos.y, 0.0f), D3DXVECTOR3(RESULT_WIDTH, RESULT_HEIGHT, 0.0f));
	m_paObject2D[2] = CObject2D::Create(D3DXVECTOR3(pos.x - 300.0f, pos.y, 0.0f), D3DXVECTOR3(RESULT_PLAYER_WIDTH, RESULT_PLAYER_HEIGHT, 0.0f));
	m_paObject2D[3] = CObject2D::Create(D3DXVECTOR3(pos.x + 400.0f, pos.y, 0.0f), D3DXVECTOR3(RESULT_PLAYER_WIDTH, RESULT_PLAYER_HEIGHT, 0.0f));

	for (int nCntResult = 0; nCntResult < MAX_TEX; nCntResult++)
	{
		// オブジェクトの親を設定
		m_paObject2D[nCntResult]->SetObjectParent(CObject::OBJECT_PARENT_RESULT);
	}
