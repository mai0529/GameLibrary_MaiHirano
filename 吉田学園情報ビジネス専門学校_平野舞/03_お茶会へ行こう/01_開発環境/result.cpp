...
bool g_ResultFade;										//フェードしているかどうか
...
void UpdateResult(void)
{
	...
	if (GetKeyboardTrigger(DIK_RETURN) == true && !g_ResultFade
		|| GetControllerPressTrigger(0, XINPUT_GAMEPAD_A) && !g_ResultFade)
	{
		SetFade(MODE_RANKING);		//ランキング画面に移行
		g_ResultFade = true;		//フェード中
	}
}
